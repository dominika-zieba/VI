...
def make_detector_response(detector_tensor, detector_vertex):
    ...
    def detector_response(f, hp, hc, ra, dec, gmst, psi):
        output = jnp.atleast_2d(antenna_response_plus(ra, dec, gmst, psi)).T*hp + jnp.atleast_2d(antenna_response_cross(ra, dec, gmst, psi)).T*hc
...
def make_get_polarization_tensor(mode):
    ...
    if mode.lower() == 'plus':
        ...
        kernel = lambda m,n: jnp.einsum('ik,jk->kij', m, m) - jnp.einsum('ik,jk->kij', n, n)    #allows for vector inputs.
    elif mode.lower() == 'cross':
        #kernel = lambda m,n: jnp.einsum('i,j->ij', m, n) + jnp.einsum('i,j->ij', n, m)
        kernel = lambda m,n: jnp.einsum('ik,jk->kij', m, n) + jnp.einsum('ik,jk->kij', n, m)  #allows for vector inputs.
    ...
    def get_polarization_tensor(ra, dec, gmst, psi):
        ...
        v = jnp.array([-jnp.sin(phi), jnp.cos(phi), jnp.zeros_like(phi)])
...
def make_antenna_response(detector_tensor, mode):
    ...
    def antenna_response(ra, dec, gmst, psi):
        ...
        return jnp.einsum('ij,kij-> k', detector_tensor, polarization_tensor)
...
def time_delay_geocentric(detector1, detector2, ra, dec, gmst):
    ...
    return jnp.atleast_2d(jnp.einsum('ik,i -> k', omega, delta_d)/  speed_of_light).T
